...
function task(config, taskName, taskFn, unparsedParamsFromArgs) {
  ...
  taskFnWithParams = function(cb, paramsFromParent) {
    ...
    if (taskFn) {
      if (taskFn._parameterized) {
        return taskFn(cb, params);
      } else {
        return taskFn(params, cb);
      }
    } else {
       var fn = config.gulp().task(taskName).unwrap();
       return fn(cb, params);
    }

  };

  taskFnWithParams.displayName = taskName;
  taskFnWithParams._parameterized = true;
